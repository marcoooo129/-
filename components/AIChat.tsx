...
const AIChat: React.FC<AIChatProps> = ({ isOpen, onClose, initialInput }) => {
  ...
  useEffect(() => {
    if (isOpen && initialInput) {
      ...
      // Don't clear if user was typing and closed/reopened accidentally unless sent
      if (isSuccess) {
        setSubject('');
        setMessage('');
        setName('');
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (formspreeId) {
      ...
      try {
        const response = await fetch(`https://formspree.io/f/${formspreeId}`, {
          ...
          body: JSON.stringify({
            ...
            message: message,
            _replyto: "no-reply@portfolio.com" // Or ask user for email field if needed
          })
        });
        ...
        console.error(err);
        setError("Something went wrong. Please try again or use the email button.");
      }
    } else {
      // Fallback to Mailto
      ...
      setIsSuccess(true); // Assume success for mailto
    }
      ...
      <div className="bg-gradient-to-b from-transparent to-black/40 relative min-h-[400px] flex flex-col">
        ...
        {isSuccess ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500">
            ...
            <h4 className="text-2xl font-black italic text-white mb-2">Message Sent!</h4>
            <p className="text-gray-400 text-sm mb-8">Thanks for reaching out. I'll get back to you at my earliest convenience.</p>
            <button 
